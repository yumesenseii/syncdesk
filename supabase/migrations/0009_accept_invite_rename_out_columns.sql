-- Definitive fix: RETURNS TABLE (workspace_id …) creates PL/pgSQL variables that
-- shadow table columns in ON CONFLICT / INSERT. Rename output columns instead of
-- relying on #variable_conflict (which may not apply on all hosted Postgres builds).
