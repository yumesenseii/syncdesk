-- Consolidate insert_activity_event_definer into ONE function (fixes ambiguous overload).
-- Migrations 0008 and 0013 each created a different signature; 5-arg calls matched both.

-- ---------------------------------------------------------------------------
-- Drop every overload of insert_activity_event_definer
-- ---------------------------------------------------------------------------
do $$
declare
  r record;
begin
  for r in
    ...
    where n.nspname = 'public'
      and p.proname = 'insert_activity_event_definer'
  loop
    execute format(
      'drop function if exists public.insert_activity_event_definer(%s) cascade',
      r.args
    );
  end loop;
end $$;
...
-- ---------------------------------------------------------------------------
-- Canonical definer helper (single definition; optional context ids at end)
-- ---------------------------------------------------------------------------
...
grant execute on function public.insert_activity_event_definer(
  uuid, uuid, text, text, jsonb, uuid, uuid, uuid
) to authenticated;

-- ---------------------------------------------------------------------------
-- Invite trigger: explicit casts (no unknown / ambiguous types)
-- ---------------------------------------------------------------------------
...
declare
  ...
  v_ws_slug text;
begin
  if NEW.status is distinct from 'pending'::text then
    return NEW;
  end if;
  ...
  perform public.insert_activity_event_definer(
    ...
    NEW.invited_by,
    'member_invited'::text,
    format('invited %s', NEW.invited_email)::text,
    jsonb_build_object(
...
-- ---------------------------------------------------------------------------
-- accept_workspace_invite: log member_joined without swallowing errors
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- log_activity_event: drop legacy 0007 overload (prevent future ambiguity)
-- ---------------------------------------------------------------------------
drop function if exists public.log_activity_event(uuid, text, text, uuid, uuid, jsonb);
